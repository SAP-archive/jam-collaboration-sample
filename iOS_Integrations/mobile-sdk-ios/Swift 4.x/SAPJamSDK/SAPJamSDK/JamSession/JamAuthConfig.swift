...
public class JamAuthConfig {
    ...
    public typealias SingleUseTokenSuccessHandler = (_ singleUseToken: String) -> Void
    ...
    public func getSingleUseToken(success: @escaping SingleUseTokenSuccessHandler, failure: ErrorHandler?) {
        ...
        _ = oauthswift!.client.post(urlStr,
            success: {
                response in
                ...
                if let match = regex.firstMatch(in: dataString as String, options:[], range: NSMakeRange(0, dataString.length)) {
                    
                    // Assigns the single use access token value from the key-value pair to a string
                    let token:String = dataString.substring(with: match.range)
                    ...
                    failure?(NSError(domain: "sapjam.error", code: -1, userInfo: errorInfo))
                }
    ...
    public func configure(server: String, key: String, secret: String, companyDomain: String? = nil) {
        ...
        if JamAuthConfig.isLoggedIn() {
            oauthswift?.client.credential.oauthToken = keychain[kAccessToken]!
            oauthswift?.client.credential.oauthTokenSecret = keychain[kAccessSecret]!
        }
    ...
    public func storeCredentials(credential: OAuthSwiftCredential) {
        keychain[kAccessToken] = credential.oauthToken
        keychain[kAccessSecret] = credential.oauthTokenSecret
    }
